    ...
    def candidate_generation_and_selection(self):
        ...
        for i in range(len(self.cluster_centers)):
            ...
            new_cand = candidate_dycors(
                ...
                xbest=self.centers[-1].x[:],
                X=np.copy(self.X),
                weights=[0.95],
                init_evals=self.exp_design.num_pts,
            ...
            uniqueness = True
            for xx in self.X:
                if scp.distance.euclidean(xx, new_cand) < 1e-6:
                    uniqueness = False
                    break
            for _, xx in self.pairs:
                if scp.distance.euclidean(xx, new_cand) < 1e-6:
                    uniqueness = False
                    break
            if uniqueness: self.pairs.append([self.centers[-1], new_cand[:]])
